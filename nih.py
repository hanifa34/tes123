...
#Login Bots
appName="CHROMEOS\t1.7.14\tChrome_OS\t1"
client = LINE('isi tokenmu', appName=appName)
...
# Pembatas Script #
                                ...
                                cctv['cyduk'][to]=True
                                settings["Sider"]=True
                                client.sendMessage(to, "cek cctv aktif")
                            elif cmd == 'ciduk off':
                                if msg.to in cctv['point']:
                                    cctv['cyduk'][to]=False
                                    settings["Sider"]=False
                                    client.sendMessage(to, "cek cctv nonaktif")
                                else:
                                    client.sendMessage(to, "Heh belom di Set")
                            elif cmd == 'mention':
                ...
                if msg.toType == 0 or msg.toType == 1 or msg.toType == 2:
                    ...
                    if msg.contentType == 0:
                        ...
                        if 'MENTION' in msg.contentMetadata.keys()!= None:
                            ...
                            names = re.findall(r'@(\w+)', clienttxt)
                            mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                            mentionees = mention['MENTIONEES']
                            lists = []
                            for mention in mentionees:
                                if clientmid in mention["M"]:
                                    if settings["autoRespon"] == True:
                                        client.sendMessage(to, "[Auto Reply]\n"+ "Hi " + cName + " 😊\n\n" + (str(settings["respon"])))
                                        sendMention(sender, "Hi @!\n[Auto Reply]\n" + (str(settings["respon"])) ,[sender])
                        ...
                        if 'MENTION' in msg.contentMetadata.keys()!= None:
                            ...
                            for mention in mentionees:
                                if clientmid in mention["M"]:
                                    if settings["autoRespon"] == True:
                                        client.sendMessage(msg.to,text=None,contentMetadata={"STKID": "110","STKPKGID": "1","STKVER": "100" }, contentType=7)
                    ...
                    if msg_id in msg_dict:
                        if msg_dict[msg_id]["from"]:
                            ...
                            if contact.displayNameOverridden != None:
                                ...
                                ret_ += "\nText : {}".format(str(msg_dict[msg_id]["clienttxt"]))
                  ...
                  if op.param1 in cctv['point']:
                      ...
                      if Name in cctv['sidermem'][op.param1]:
                          ...
                          if " " in Name:
                              nick = Name.split(' ')
                              if len(nick) == 2:
                                  client.sendMessage(op.param1, "cyduk " + "☞ " + Name + " ☜" + "\nhallo kang minyak 🙊🙊🙊 ")
                                        
                              else:
                                  client.sendMessage(op.param1, "cyduk " + "☞ " + Name + " ☜" + "\nhallo kang minyak 🙊🙊🙊 ")
                                        
                          else:
                              client.sendMessage(op.param1, "cyduk " + "☞ " + Name + " ☜" + "\ntuhkan sider nya nambah  (-__-)")
